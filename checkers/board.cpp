...
void Board::setupBoard(std::vector<string> &white,std::vector<string> &whiteTex, std::vector<string> &black, std::vector<string> &blackTex)
{
    whiteCoordinates.clear();
    blackCoordinates.clear();
    whiteTextures.clear();
...
void Board::displayBoard(sf::RenderWindow &window) {
    sf::Texture board;
    sf::Texture* textureWhite=new sf::Texture [whiteCoordinates.size()];
    sf::Texture* textureBlack=new sf::Texture [blackCoordinates.size()];
    sf::Sprite boardS;
    sf::Sprite* white_pawnS=new sf::Sprite [whiteCoordinates.size()];
    sf::Sprite* black_pawnS=new sf::Sprite [blackCoordinates.size()];
    board.loadFromFile("board.jpg");
    /*white_pawn.loadFromFile("white_pawn.png");
    black_pawn.loadFromFile("black_pawn.png");
    black_king.loadFromFile("black_king.png");
    white_king.loadFromFile("black_pawn.png");*/
    ...
    for(int i=0;i<whiteCoordinates.size();i++) {
        ...
        white_pawnS[i].setPosition(convertX(whiteCoordinates[i][0]),convertY(whiteCoordinates[i][1]));
        white_pawnS[i].setScale(0.18,0.18);
    ...
    for(int i=0;i<blackCoordinates.size();i++) {
        ...
        black_pawnS[i].setPosition(convertX(blackCoordinates[i][0]),convertY(blackCoordinates[i][1]));
        black_pawnS[i].setScale(0.18,0.18);

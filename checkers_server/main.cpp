...
static void handle_command_prepare(int clientFd, const std::string &line) {
    ...
    if (line.rfind("NICK ", 0) == 0) {
        ...
        queueSend(clientFd, "Dołączono do gry.\n");
        }
        else
        {
        ...
        else if(line == "UNREADY" || line == "UNREADY\n")
        {
            ready_players--;
                std::printf("Klient %s nie jest gotowy\n", name);
                clientChoice.erase(clientFd);
                broadcastPlayers();
                queueSend(clientFd, "OK\n");
        }
...
static void handle_command(int clientFd, const std::string &line,string &wynik) {
    ...
    if(line=="CANCEL_VOTE\n" || line=="CANCEL_VOTE")
    {
        if(spectators.find(clientFd)==spectators.end())
        {
            if(game.getCurrentPlayer()==clientColors[clientFd])
            {
                if(clientChoice.find(clientFd)!=clientChoice.end())
                {
                    votes[clientChoice[clientFd]]--;
                    if(votes[clientChoice[clientFd]]==0)
                    {
                        votes.erase(clientChoice[clientFd]);
                    }
                    MoveLog log;
